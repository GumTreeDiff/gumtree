...
public class TreeMetricsProvider implements MetricProvider<TreeMetricsProvider.TreeMetrics> {
    ...
    private class TreeMetricComputer extends TreeVisitor.InnerNodesAndLeavesVisitor {
        ...
        public void endInnerNode(ITree tree) {
            ...
            for (ITree child : tree.getChildren()) {
                ...
                currentHash += metrics.hash * Factory.hashFactor(exponent);
        ...
        private int innerNodeHash(ITree tree, int size, int middleHash) {
            return Objects.hash(tree.getType(), tree.getLabel(), Factory.ENTER)
                    + middleHash
                    + Objects.hash(tree.getType(), tree.getLabel(), Factory.LEAVE) * Factory.hashFactor(size);
        }
    ...
    public static class Factory implements MetricProviderFactory<TreeMetrics> {
