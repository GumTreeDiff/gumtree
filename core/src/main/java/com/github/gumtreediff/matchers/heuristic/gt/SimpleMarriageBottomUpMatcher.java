...
public class SimpleMarriageBottomUpMatcher implements Matcher {
    ...
    public MappingStore match(Tree src, Tree dst, MappingStore mappings) {
        for (Tree t : src.postOrder()) {
            if (t.isRoot()) {
                ...
                Tree bestDst = bestDstCandidate(mappings, t);
                if (bestDst != null && bestSrcCandidate(mappings, bestDst) == t) {
                    lastChanceMatch(mappings, t, bestDst);
                    mappings.addMapping(t, bestDst);
                }
            }
    ...
    protected Tree bestDstCandidate(MappingStore mappings, Tree src) {
        List<Tree> candidates = getDstCandidates(mappings, src);
        Tree dst = null;
        var maxSim = -1D;
        var tSize = src.getDescendants().size();

        for (var candidate : candidates) {
            var threshold = Double.isNaN(simThreshold)
                    ? 1D / (1D + Math.log(candidate.getDescendants().size() + tSize))
                    : simThreshold;
            var sim = SimilarityMetrics.chawatheSimilarity(src, candidate, mappings);
    ...
    protected Tree bestSrcCandidate(MappingStore mappings, Tree dst) {
        List<Tree> candidates = getSrcCandidates(mappings, dst);
        Tree src = null;
        var maxSim = -1D;
        var tSize = dst.getDescendants().size();

        for (var candidate : candidates) {
            var threshold = Double.isNaN(simThreshold)
                    ? 1D / (1D + Math.log(candidate.getDescendants().size() + tSize))
                    : simThreshold;
            // swap candidate and dst as chawatheSimilarity is not symmetric
            var sim = SimilarityMetrics.chawatheSimilarity(candidate, dst, mappings);
            if (sim > maxSim && sim >= threshold) {
                maxSim = sim;
                src = candidate;
            }
    ...
    protected List<Tree> getSrcCandidates(MappingStore mappings, Tree dst) {
        List<Tree> seeds = new ArrayList<>();
        for (Tree c : dst.getDescendants()) {
            Tree m = mappings.getSrcForDst(c);
            if (m != null)
                seeds.add(m);
        }
        List<Tree> candidates = new ArrayList<>();
        ...
        for (var seed : seeds) {
            while (seed.getParent() != null) {
                ...
                if (visited.contains(parent))
                    break;
                visited.add(parent);
                if (parent.getType() == dst.getType() && !mappings.isSrcMapped(parent) && !parent.isRoot())
                    candidates.add(parent);
                seed = parent;
            }
        }
