...
#'                       show.csep=T, show.rsep=T, show.box=T, separator="grey95", name="test"))
#'  grid.edit("test",gp=gpar(fontsize=8, lwd=0.1),  equal.width=TRUE, grep=TRUE, global=TRUE)
#' \dontrun{ 
#' pdf("test2.pdf", height=50)
#' print(system.time( grid.table(iris)) ) # about 8s here
#' dev.off()
#' }
...
tableGrob <- function(d, rows=rownames(d), cols=colnames(d),
  ...
  with(theme, # params provided as a list
   gTree(d=d, rows= if(show.rownames) rows, cols=if(show.colnames) cols,
...
theme.list <- function(...)
  updatelist(
  list(show.rownames=FALSE, show.colnames=FALSE,separator=NA,
...
theme.blank <- function(...)
  updatelist(
  list(gpar.corefill=gpar(col=NA),separator=NA, 
       show.csep=FALSE, show.rsep=FALSE,
       gpar.corefill = gpar(fill = NA, col=NA), 
       gpar.rowfill = gpar(fill = NA, col=NA), 
       gpar.colfill = gpar(fill = NA, col=NA)), list(...))
...
theme.default <- function(...)
  updatelist(
  list(show.rownames=TRUE, show.colnames=TRUE,
       row.just="center", col.just="center", core.just="center", 
       separator="white", show.box=FALSE, show.csep=FALSE, show.rsep=FALSE,
       equal.width = FALSE, equal.height=FALSE, 
       padding.h = unit(4, "mm"), padding.v=unit(4, "mm"), 
