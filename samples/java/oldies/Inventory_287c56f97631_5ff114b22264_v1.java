...
public class Inventory {
	 ...
	 *
	 * @param filter the <code>Item</code> <code>Filter</code> to use
	 * @return an array instance of <code>Item</code>
	 */
	...
	/**
	 * Gets all the inventory items matching with any of the provided names.
	 *
		...
		return getItems(new Filter<Item>() {
			public boolean accept(final Item item) {
				for (String name : names) {
					if (name != null && item.getName() != null && item.getName().contains(name)) {
						return true;
					}
				}
				return false;
