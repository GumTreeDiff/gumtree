...
def plotCsv(file):
  ...
  plot = ggplot(data, aes(x='factor(algorithm)', y='runtime / 1_000_000_000', fill='factor(algorithm)')) + geom_violin(draw_quantiles=[0.25, 0.5, 0.75], show_legend=False) + scale_y_continuous(trans="log1p", breaks = [0, 0.5, 1, 10, 100, 1000]) + ylab("Runtime (s)") + xlab("Algorithm") + scale_fill_manual(values=cb_palette()) + theme_light()
