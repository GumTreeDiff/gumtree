...
public class ActionsCollector {
    ...
    private final static String OUTPUT_DIR = "build/tmp/actions/";

    private final static String RES_DIR = "src/jmh/resources/";

    private final static String REF_DIR = "src/main/resources/";
    ...
    public static void collectActions() throws Exception {
        Run.initGenerators();
        List<Path> paths = Files.walk(Paths.get(RES_DIR)).filter(p -> p.getFileName().toString().matches(".*_v0_.*\\.xml")).collect(Collectors.toList());
        ...
        for (Path path : paths) {
            ...
            CompositeMatchers.ClassicGumtree matcher = new CompositeMatchers.ClassicGumtree(src.getRoot(), dst.getRoot(), new MappingStore());
            matcher.match();
