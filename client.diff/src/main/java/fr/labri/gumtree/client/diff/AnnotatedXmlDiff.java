...
@Register(name = "axmldiff", description = "Dump annotated xml tree", experimental = true, options = AbstractDiffClient.Options.class)
public abstract class AnnotatedXmlDiff extends AbstractDiffClient<AnnotatedXmlDiff.Options> {
    ...
    static class Options extends AbstractDiffClient.Options{
        ...
        public Option[] values() {
            return Option.Context.addValue(super.values(),
                    ...
                    new Option("--dst", String.format("Dump destination tree (default: %s)", !isSrc ? "yes": "no"), 0) {
                        @Override
                        protected void process(String name, String[] args) {
                            isSrc = false;
                        }
    ...
    public void run() {
        ...
        try {
            TreeIoUtils.toAnnotatedXml((opts.isSrc) ? getSrcTreeContext() : getDstTreeContext(), opts.isSrc, m.getMappings()).writeTo(System.out);
        } catch (Exception e) { }
    }
}
